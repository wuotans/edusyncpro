...
export default function SchoolAdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Olá, {user?.full_name || 'Admin'}</h1>
        <p className="text-sm text-slate-500 mt-1">Painel da escola</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <Link to={s.link} key={s.label} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="divide-y divide-slate-50 max-h-64 overflow-y-auto">
              {classes.map(c => (
                <div key={c.id} className="px-5 py-3 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-400">{c.shift === 'morning' ? 'Manhã' : c.shift === 'afternoon' ? 'Tarde' : 'Noite'}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
          <div className="p-5 border-b border-slate-50">
            <h2 className="text-base font-semibold text-slate-900">Professores</h2>
          </div>
          {teachers.length === 0 ? (
            <div className="p-8 text-center">
              <p className="text-sm text-slate-400">Nenhum professor cadastrado</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-50 max-h-64 overflow-y-auto">
              {teachers.map(t => (
                <div key={t.id} className="px-5 py-3 flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-cyan-50 flex items-center justify-center text-xs font-bold text-cyan-600">
                    {(t.full_name || 'P')[0]}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-slate-700">{t.full_name || t.email}</p>
