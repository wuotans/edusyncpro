...
export default function Observations() {
  ...
  const load = async () => {
    ...
    try {
      const [obs, a, c, s, st] = await Promise.all([
        base44.entities.Observation.filter({ teacher_id: user.id }),
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ student_id: '', class_id: classes[0]?.id || '', subject_id: '', content: '', type: 'academic', date: new Date().toISOString().split('T')[0] });
    setDialogOpen(true);
  };

  const openEdit = (o) => {
    setEditing(o);
    setForm({ student_id: o.student_id, class_id: o.class_id || '', subject_id: o.subject_id || '', content: o.content, type: o.type || 'academic', date: o.date || '' });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await base44.entities.Observation.update(editing.id, form);
        toast({ title: 'Observação atualizada' });
      } else {
        await base44.entities.Observation.create({ ...form, teacher_id: user.id, school_id: schoolId });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await base44.entities.Observation.delete(id);
  ...
  const filtered = observations.filter(o => {
    ...
    const matchSearch = (student?.name || '').toLowerCase().includes(search.toLowerCase()) || o.content.toLowerCase().includes(search.toLowerCase());
    const matchClass = filterClass === 'all' || o.class_id === filterClass;
    return matchSearch && matchClass;
  }).sort((a, b) => (b.date || b.created_date || '').localeCompare(a.date || a.created_date || ''));
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{observations.length} observações</p>
        </div>
        <Button onClick={openCreate} className="bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 text-white rounded-xl shadow-sm">
          <Plus className="w-4 h-4 mr-2" /> Nova Observação
        </Button>
      </div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar por aluno ou conteúdo..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 rounded-xl border-slate-200" />
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center">
          ...
          <p className="text-sm text-slate-400">Nenhuma observação encontrada</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className="text-sm font-semibold text-slate-900">{getStudentName(o.student_id)}</span>
                    {o.class_id && <span className="text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-cyan-50 text-cyan-700">{getClassName(o.class_id)}</span>}
                    <span className={`text-[11px] font-medium px-2.5 py-0.5 rounded-full ${typeColors[o.type] || typeColors.other}`}>
                      {typeLabels[o.type] || o.type}
                    </span>
                  </div>
                  <p className="text-sm text-slate-600 whitespace-pre-wrap">{o.content}</p>
                  {o.date && (
                ...
                <div className="flex items-center gap-1 ml-3">
                  <button onClick={() => openEdit(o)} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600"><Edit className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(o.id)} className="p-2 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="rounded-2xl max-w-lg">
          <DialogHeader><DialogTitle>{editing ? 'Editar Observação' : 'Nova Observação'}</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-4">
            ...
            <div>
              <label className="text-xs font-medium text-slate-600 mb-1.5 block">Observação *</label>
              <Textarea value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} rows={4} className="rounded-xl resize-none" placeholder="Descreva a observação sobre o aluno..." />
            </div>
            <Button onClick={handleSave} className="w-full bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 text-white rounded-xl">
              {editing ? 'Salvar' : 'Registrar Observação'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
