...
import { Users, Plus, Edit, Search, Mail } from 'lucide-react';
...
export default function UsersManagement() {
  ...
  const load = async () => {
    try {
      const [u, s] = await Promise.all([
        ...
        isSuperAdmin ? base44.entities.School.list() : Promise.resolve([])
      ]);
      setUsers(u);
      setSchools(s);
  ...
  const handleSave = async () => {
    try {
      if (editingUser) {
        await base44.entities.User.update(editingUser.id, { role: form.role, school_id: form.school_id || null });
        toast({ title: 'Usuário atualizado' });
      } else {
        if (!form.email.trim()) return;
        await base44.users.inviteUser(form.email, form.role);
        // After invite, update user's school_id
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{filtered.length} usuários</p>
        </div>
        <Button onClick={openInvite} className="bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 text-white rounded-xl shadow-sm">
      ...
      <div className="relative">
        ...
        <Input placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 rounded-xl border-slate-200" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-slate-50">
            {filtered.map(u => (
              <div key={u.id} className="px-5 py-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-100 to-cyan-50 flex items-center justify-center text-sm font-bold text-cyan-600 flex-shrink-0">
                    {(u.full_name || u.email || 'U')[0].toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-slate-700 truncate">{u.full_name || 'Sem nome'}</p>
                    <p className="text-xs text-slate-400 truncate">{u.email}</p>
                    {isSuperAdmin && u.school_id && <p className="text-[11px] text-cyan-500 mt-0.5">{getSchoolName(u.school_id)}</p>}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {getRoleBadge(u.role)}
                  <button onClick={() => openEdit(u)} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors">
          ...
          <div className="space-y-4 mt-4">
            {!editingUser && (
              <div>
                ...
                <Input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="email@exemplo.com" className="rounded-xl" />
              </div>
            )}
            <div>
              <label className="text-xs font-medium text-slate-600 mb-1.5 block">Papel</label>
              <Select value={form.role} onValueChange={v => setForm({ ...form, role: v })}>
                <SelectTrigger className="rounded-xl"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {isSuperAdmin && <SelectItem value="school_admin">Admin Escola</SelectItem>}
            ...
            {isSuperAdmin && (
              <div>
                ...
                <Select value={form.school_id} onValueChange={v => setForm({ ...form, school_id: v })}>
                  <SelectTrigger className="rounded-xl"><SelectValue placeholder="Selecione a escola" /></SelectTrigger>
                  <SelectContent>
