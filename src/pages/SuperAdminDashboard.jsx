  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Painel geral do sistema</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
        <div className="p-5 border-b border-slate-50 flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-900">Escolas Recentes</h2>
          <Link to="/schools" className="text-xs font-medium text-cyan-600 hover:text-cyan-700">Ver todas →</Link>
        </div>
        {schools.length === 0 ? (
          <div className="p-12 text-center">
            <School className="w-10 h-10 text-slate-200 mx-auto mb-3" />
            <p className="text-sm text-slate-400">Nenhuma escola cadastrada</p>
            <Link to="/schools" className="text-xs text-cyan-600 mt-2 inline-block">Adicionar escola</Link>
          </div>
        ) : (
          <div className="divide-y divide-slate-50">
            {schools.slice(0, 5).map((school) => (
              <div key={school.id} className="px-5 py-3.5 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{school.email || 'Sem email'}</p>
                  </div>
                </div>
...
function DashboardSkeleton() {
  return (
    <div className="space-y-8 animate-pulse">
      <div><div className="h-7 bg-slate-100 rounded-lg w-48" /><div className="h-4 bg-slate-100 rounded w-32 mt-2" /></div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[1,2,3].map(i => <div key={i} className="bg-white rounded-2xl p-5 h-28 border border-slate-100" />)}
      </div>
